...
type EventoDetalleView = "general" | "materials" | "costs";
...
export default function EventoDetalle() {
  ...
  const [activeView, setActiveView] = useState<EventoDetalleView>("general");
  ...
  const tax = (subtotal / 100) * (booking.tax ?? 0);
  ...
  return (
    <div className="mx-auto flex w-full min-w-0 max-w-6xl flex-col gap-6 p-4 md:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <PDFDownloadLink
              ...
              fileName={`fixture-${client?.name ?? ""}-${client?.lastName ?? ""}.pdf`.toLowerCase().replace(/\s+/g, "-")}
            >
              {({ loading }) => (
                <Button
                  type="button"
            ...
            {isEditMode ? (
              editingTime ? (
                ...
                <button
                  ...
                  onClick={() => {
                    setTimeStart(booking.start_time?.slice(0, 5) ?? "");
                    setTimeEnd(booking.end_time?.slice(0, 5) ?? "");
                    setEditingTime(true);
                  }}
                >
                  ...
                  {booking.start_time
                    ...
                    : <span className="text-xs italic">Sin horario</span>}
                </button>
              )
            ) : (
              <span className="flex items-center gap-1.5">
      ...
      <div
        ...
        role="group"
        aria-label="Vistas del detalle de la reserva"
      >
        {(
          [
      ...
      {/* Vista: Materiales y mano de obra */}
      {activeView === "materials" && (
        <>
          ...
          <div className="min-w-0 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
            ...
            {isEditMode && hasIncompleteTimes && (
              <p className="border-b border-border px-5 py-2 text-xs text-amber-700 dark:text-amber-300">
